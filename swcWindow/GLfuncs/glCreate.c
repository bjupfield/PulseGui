...
/**
 * @brief Create a Program object || Placeholder
 * 
 * 
...
uint32_t createVAO(programNames* programName, swcWin *win)
{
    ...
    for(uint8_t i = 0; i < programName->vaoAlignment; i++)
    {
        win->glPointers.sigGetActiveAttrib(programName->programName, i, 256, &(info[i].length), &(info[i].size), &(info[i].type), name);
...
uint32_t retrieveGLFuncs(swcWin *win)
{
    ...
    procMacro(sigGetAttribLocation, "glGetAttribLocation", win->glPointers);
    procMacro(sigVertexArrayAttribBinding, "glVertexArrayAttribBinding", win->glPointers);
    procMacro(sigVertexArrayVertexBuffer, "glVertexArrayVertexBuffer", win->glPointers);
    procMacro(sigVertexArrayAttribFormat, "glVertexArrayAttribFormat", win->glPointers);
